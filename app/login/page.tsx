...
export default function LoginPage() {
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    if (error || !data.user) {
      ...
      if (
        authMessage.includes("database error granting user") ||
        authMessage.includes("unexpected_failure")
      ) {
        showError(
          "No se pudo iniciar sesion. Si ya te registraste, espera aprobacion del owner."
        );
        return;
      }
    ...
    if (profileError) {
      ...
      if (
        profileMessage.includes("column") &&
        profileMessage.includes("approved")
      ) {
        showError(
          "Error de base de datos: aun hay una policy usando 'approved'. Debe usar 'is_approved'."
        );
        return;
      }
      if (
        ...
        showError(
          "No se pudo leer tu perfil por permisos (RLS). Revisa las policies de profiles."
        );
        return;
      }
    ...
    if (!profile) {
      await supabase.auth.signOut();
      showError(
        "No se encontro tu perfil o aun no tienes permisos. Si te acabas de registrar, espera aprobacion."
      );
      return;
    }
    ...
    // No mantener sesion activa antes de validar el PIN.
      ...
      <main className={styles.cardWrap}>
        <div className={styles.card}>
          ...
          {step === "credentials" ? (
            ...
            <div className={styles.pinCard}>
              ...
              <p className={styles.pinText}>
                Tus datos son correctos. Ingresa ahora tu codigo de compra.
              </p>
              <form className={styles.form} onSubmit={handlePinLogin}>
                ...
                <div className={styles.pinActions}>
                  ...
                  >
                    {isSubmittingPin ? "Validando..." : "Entrar a dashboard"}
                  </button>
                  <button
          ...
          <p className={styles.link}>
            No tienes cuenta?{" "}
            <span onClick={() => router.push("/register")}>Registrate</span>
          </p>
        </div>
      </main>
    </div>
